metaBin_16S_correlation_sq=function(bins_metadata_mat=bins_metadata_mat,bins_raw_mat=bins_raw_mat,S16_file_mat=S16_file_mat)
{
  ...
  {
    print(i)
    Tn.X=sum(bins_raw_mat[rownames(bins_metadata_mat),i]/(as.numeric(bins_metadata_mat[,"length"])-100+1))
    bins_tpm[,i]=(((bins_raw_mat[rownames(bins_metadata_mat),i]/(as.numeric(bins_metadata_mat[,"length"])-100+1))/Tn.X)*1e+06)
